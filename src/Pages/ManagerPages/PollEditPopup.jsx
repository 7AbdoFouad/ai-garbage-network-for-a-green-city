...
export default function PollEditPopup({ poll, onSave, onClose }) {
  const formik = useFormik({
    ...
    onSubmit: (data) => {
      const correctFormLink = data.pollFormLink.includes("&lang=en")
        ? data.pollFormLink
        : data.pollFormLink + "&lang=en";
      const updatedData = {
        ...poll,
        ...data,
  ...
  useEffect(() => {
    ...
    // منع تمرير الصفحة الرئيسية عند ظهور الـ popup
    ...
    <div className={styles.overlay} onClick={onClose}>
      <div className={styles.modalContainer} onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={formik.handleSubmit}>
          <label htmlFor="pollName" className={styles.label}>
            Poll Name:
          </label>
          <input
            type="text"
            ...
            className={`form-control ${styles.input} ${
              formik.touched.pollName && formik.errors.pollName ? "is-invalid" : ""
            }`}
          />
          ...
          <label htmlFor="pollDesc" className={styles.label}>
            Poll Description:
          </label>
          <textarea
            name="pollDesc"
          ...
          <label htmlFor="pollEndDate" className={styles.label}>
            End Date:
          </label>
          <input
            type="date"
            ...
            onBlur={formik.handleBlur}
            className={`form-control ${styles.input} ${
              formik.touched.pollEndDate && formik.errors.pollEndDate ? "is-invalid" : ""
            }`}
          />
          ...
          <label htmlFor="pollFormLink" className={styles.label}>
            Form Link:
          </label>
          <input
            type="text"
            ...
            onBlur={formik.handleBlur}
            className={`form-control ${styles.input} ${
              formik.touched.pollFormLink && formik.errors.pollFormLink ? "is-invalid" : ""
            }`}
          />
          ...
          {formik.values.imgFile && (
            <div className="mt-2">
              <img
                ...
                style={{ maxWidth: "100%", height: "auto", background: "#1bad1d" }}
              />
            </div>
          )}
