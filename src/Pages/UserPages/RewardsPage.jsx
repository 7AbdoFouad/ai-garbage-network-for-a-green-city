  ...
  return (
    <>
      {requirements.numOfAcceptedAnnouncements && (
        <li> عدد البلاغات المقبولة: {requirements.numOfAcceptedAnnouncements}✅</li>
      )}
      {requirements.numOfCompletedActivities && (
        <li> عدد الفعاليات المكتملة: {requirements.numOfCompletedActivities}✅</li>
      )}
      {requirements.numOfCompletedPolls && (
        <li> عدد استطلاعات الرأي المكتملة: {requirements.numOfCompletedPolls}✅</li>
      )}
    </>
  );
}
      ...
      <div className={styles.rewardGrid}>
        {Rewards.map((reward, index) => (
          <div key={index} className={styles.rewardCard}>
            {/* Reward Image */}
            <img  src={`/src/Pages/UserPages/${index+8}.jpg`} alt={reward.title} className={styles.rewardImage} />
            ...
            <div className={styles.rewardContent}>
              ...
              {/* Reward Details */}
              <p className={styles.rewardDetails}>🗓️ <strong>تاريخ الانتهاء:</strong> {reward.ExpiryDate}</p>
              {/* <p className={styles.rewardDetails}>🔹 <strong>حالة المكافأة:</strong> {reward.status}</p> */}
              <p className={styles.rewardValue}>💰 <strong>قيمة المكافأة:</strong> {reward.rewardValue} دولار</p>

              {/* Claim Reward Button */}
              <button className={styles.claimButton}>🎁 احصل على المكافأة</button>
            </div>
          </div>
        ))}
      </div>
